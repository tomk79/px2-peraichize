...
module.exports = function(main){
	...
	this.init = function(targetDiv){
		...
		let $htmlSearchForm = $(
			`<div class="px2-peraichize">
				<form action="javascript:;" method="get" id="px2-peraichize__search-form">
				...
				<div class="px2-peraichize__result"></div>
			</div>`
		);
		...
		// 検索入力のイベントリスナー
		$targetDiv.find('#px2-peraichize__search-form')
			.on('submit', function(e){
				...
				main.search(strKeywords, function(results, documentList){
					const $resultsDiv = $targetDiv.find(`.px2-peraichize__result`);
					$resultsDiv.html("");
					...
					if (results.length === 0) {
						...
						results.forEach((result) => {
							...
							listItem.innerHTML = `
								<p class="px2-peraichize__result-title"><a href="${href_prefix}${documentList.contents[result.id].h}">${title}</a></p>
								<p class="px2-peraichize__result-summary">${content}</p>
							`;
